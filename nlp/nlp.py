...
def train_model(
    ...
    history = model.fit(x_tr, y_tr, epochs=num_epochs, batch_size=batch_size,
...
def evaluate_model(x_tst, y_tst, text, tokenizer, sen_pad_len,
                   doc_pad_len, model):
...
if __name__ == "__main__":
    ...
    documents_x_tr = np.array([np.array(lst, dtype='float')
                               for lst in documents_x_tr], dtype='object')
    documents_x_tst = np.array([np.array(lst, dtype='float')
    ...
    # now all docs have the same number of sentences
    ...
    if model is None:
        ''' # TODO (maybe): model = load_model(path,
    ...
    evaluate_model(x_tst, y_tst,
                   texts[4], tokenizer, sen_pad_len,
                   doc_pad_len, model)
